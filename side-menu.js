(function () {
  // Wire up the mobile side menu, including focus management and body scroll locking.
  const website = document.querySelector('.mc-website');
  const sidebar = document.getElementById('mc-side-menu');
  const mainHeader = document.getElementById('mc-mainHeader');
  const stickyHeaderContent = document.getElementById('mc-stickyHeader');
  ...
  function getFocusableInSidebar() {
    // Only return elements users can actually tab to right now.
  ...
  function syncAria(expanded) {
    // Keep assistive-technology state in sync with the visible menu state.
  ...
  function lockBodyScroll() {
    // Prevent page scrolling while the overlay menu is open, without causing layout shift.
  ...
  function openMenu() {
    // Apply all classes/attributes needed to show the menu and move focus into it.
    isOpen = true;
  ...
  function closeMenu() {
    // Revert classes/attributes and release body scroll when menu closes.
    isOpen = false;
  ...
  function toggleMenu() {
    // Single toggle entry point used by both hamburger buttons.
    if (isOpen) {
      closeMenu();
      return;
    }
  ...
  [mainHeader, stickyHeader, stickyHeaderContent].filter(Boolean).forEach(function (header) {
  ...
  document.addEventListener('keydown', function (event) {
    // Escape closes the menu; Tab stays trapped inside while open.
